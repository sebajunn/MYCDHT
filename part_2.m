...
% cases:   Bearing Only and Range+Bearing (as required in a subsequent
% project). (i.e. no landmarks???)
...
n_usedLanmarks = 3 ;    %it must be : 0 < n_usedLanmarks <5, but you can modify the function "CreateSomeMap()" to extend it.
...
for i=1:Li,     % loop
    ...
    Xdr    = RunProcessModelDr(Xdr,Noisy_speed,Noisy_GyroZ,Dt) ;
    ...
    J = [ [1,0,-Dt*Noisy_speed*sin(Xe(3)), 0 ]  ; [0,1,Dt*Noisy_speed*cos(Xe(3)),0] ; ...   
        [ 0,0,1, -Dt ]; [0,0,0,1] ] 
    % then I calculate the new coveraince, after the prediction P(K+1|K) = J*P(K|K)*J'+Q 
    F_u = Dt*[[Noisy_speed*cos(Xe(3)) 0] ; [Noisy_speed*sin(Xe(3)), 0 ] ; [ 0,1 ]; [0,0];];
    
    Q_u = F_u*diag( [sdev_rangeMeasurement*sdev_rangeMeasurement*4, stdDevGyro*stdDevGyro] )*F_u';
    P = J*P*J' + Q + Q_u;
   ...
   % The precition step, for this iteration, is done.
    ...
    if nDetectedLandmarks>0,     % any laser data and detected landmarks?
        ...
        for u=1:nDetectedLandmarks,
            ...
            % some auxiliary variables. (euclidean distances!!)
            ...
            H = [ [  -eDX/eDD , -eDY/eDD , 0, 0 ] ; [eDY/(eDX*eDX + eDY*eDY) , -eDX/(eDX*eDX + eDY*eDY) , -1, 0] ];  % Jacobian of h(X); size 2x3
            ...
            ExpectedAngle = atan2(NavigationMap.landmarks(ID,2) - Xe(2),NavigationMap.landmarks(ID,1)-Xe(1)) - Xe(3) + pi/2;
            
            z  = [MasuredRanges(u) - ExpectedRange;min(MeasuredBearing(u)-ExpectedAngle,2*pi-(MeasuredBearing(u)-ExpectedAngle))];

            % ------ covariance of the noise/uncetainty in the measurements
            %R = sdev_rangeMeasurement*sdev_rangeMeasurement*4 ;
            R = diag( [sdev_rangeMeasurement*sdev_rangeMeasurement*4, stdDevGyro*stdDevGyro] );
...
% PLOT some results. 
SomePlots(Xreal_History,Xe_History,XeDR_History,NavigationMap, bias_plot) ;
...
function Xnext=RunProcessModelDr(X,speed,GyroZ,dt) 
...
function [ranges,bearing,IDs] = GetMeasurementsFomNearbyLandmarks(X,map)
    ...
    if map.nLandmarks>0,
        ...
        bearing = atan2(map.landmarks(:,2) - X(2),map.landmarks(:,1)-X(1)) - X(3) + pi/2;
...
function [speed,GyroZ] = SimuControl(X,t)
    ...
    GyroZ = 3*pi/180 + sin(0.1*2*pi*t/50)*.02 + 1;         % some crazy driver moving the steering wheel...
return ;
...
function InitSimulation(stdDevSpeed,stdDevGyro,sdev_rangeMeasurement,DtObservations)
    ...
    ContextSimulation.Xreal = [ 0; 0;pi/2;1 ] ;     % [x;y;phi]
...
function [nDetectedLandmarks,MasuredRanges,MeasuredBearing, IDs]=GetObservationMeasurements(map)
    ...
    % simulated platform.
        ...
        % this is the noise:
        noiseInRangeMeasurements= ContextSimulation.sdev_rangeMeasurement*randn(size(RealRanges));
        ...
        noiseInBearingMeasurements = ContextSimulation.stdDevGyro*randn(size(RealBearings));
        MeasuredBearing = RealBearings + noiseInBearingMeasurements;
...
function SomePlots(Xreal_History,Xe_History,Xdr_History,map, bias_plot) ;
...
figure(5); clf;
plot([1:length(bias_plot)],bias_plot,'m') ;
return ;
% -------------------------------------------------------------------------
