...
while (1)
   ...
   % end
    ...
    j_1 = [1,0,-Dt*Xe(5)*sin(Xe(3)),0,Dt*cos(Xe(3))];
    j_2 = [0,1,Dt*Xe(5)*cos(Xe(3)),0,Dt*sin(Xe(3))];
    j_3 = [0,0,1, -1*Dt, 0];
    j_4 = [0,0,0,1, 0];
    j_5 = [0,0,0,0, 1];
   ...
   % J = [j_1 ;j_2 ;j_3; j_4] ;
    ...
    for j = 1:M
        ...
        h_1 = [  -eDX/(sqrt( eDX*eDX + eDY*eDY )) , -eDY/(sqrt( eDX*eDX + eDY*eDY )) , 0 , 0, 0];
        h_2 = [eDY/((sqrt( eDX*eDX + eDY*eDY ))^2), -eDX/((sqrt( eDX*eDX + eDY*eDY ))^2), -1, 0, 0];
        
	    H = [h_1;h_2];
		     
        % Jacobian of h(X); size 2x5
    ...
    if (trace_flag == 0)
        set(handles.velocity_EKF,'xdata',times-times(1));
        set(handles.velocity_EKF,'ydata', Xe_History(5,1:length(times)));
        
    end
    if (trace_flag == 0)
        
        set(handles.velocity_real,'xdata',times-times(1));
       % set(handles.velocity_real,'ydata',velocity_array(1:length(times)));
...
function new_state = int_state(omega, dt, old_state)
        new_state = zeros(1,3);
        phi = old_state(3);
        x = old_state(1);
        y = old_state(2);
		...
		new_state(1) = v*cos(phi)*dt + x; 
		new_state(2) = v*sin(phi)*dt + y; 
