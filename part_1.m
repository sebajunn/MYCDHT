...
for i=1:Li,     % loop
    ...
    %QU = FU*diag( [sdev_rangeMeasurement*sdev_rangeMeasurement*4, stdDevGyro] )*FU';
    ...
    FU = [Dt*cos(Xe(3)) 0; Dt*sin(Xe(3)) 0; 0 Dt];
    QU = FU * diag( [sdev_rangeMeasurement*sdev_rangeMeasurement, stdDevGyro*stdDevGyro] ) * FU';
    P = J*P*J'+ Q + QU;
   ...
   % The precition step, for this iteration, is done.
    ...
    if nDetectedLandmarks>0,     % any laser data and detected landmarks?
        ...
        for u=1:nDetectedLandmarks,
            ...
            eDS = eDX*eDX + eDY*eDY;
            eDD = sqrt( eDS ) ; %   so : sqrt( (xu-x)^2+(yu-y)^2 ) 
    
            
            
            % here is it. "H". I reuse some previous calculations.
            H = [ [  -eDX/eDD , -eDY/eDD , 0 ] ; [eDY/eDS , -eDX/eDS , -1] ];  % Jacobian of h(X); size 2x3
            ...
            ExpectedBearing = atan2(NavigationMap.landmarks(ID,2) - Xe(2),NavigationMap.landmarks(ID,1)-Xe(1)) - Xe(3) + pi/2;
            ...
            zval = min(x-y,2*pi-(x-y));

            z  = [MasuredRanges(u) - ExpectedRange;zval];
            
            % ------ covariance of the noise/uncetainty in the measurements
            %R = sdev_rangeMeasurement*sdev_rangeMeasurement*4 ;
            R = diag( [sdev_rangeMeasurement*sdev_rangeMeasurement*4, stdDevGyro] );
