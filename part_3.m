...
for i = 1:length(IMU_times)-1
    ...
    v = velocity(i);
    my_state = state(i,:);
    state(i+1,:) = int_state(w,v,dt,my_state);
end

set(handles.att_trace,'xdata',IMU_times(1:i));
set(handles.att_trace,'ydata',(180/pi)*state(1:i,3));

set(handles.xy_trace,'xdata',state(:,1));
set(handles.xy_trace,'ydata',state(:,2));
...
for i = 2:N;
    ...
    j_1 = [1,0,-Dt*encoder_speed*sin(Xe(3))];
    j_2 = [0,1,Dt*encoder_speed*cos(Xe(3))];
    j_3 = [ 0,0,1];
    ...
    if (trace_flag == 0)
        set(handles.vehicle_trace,'xdata',state(1:i,1));
        set(handles.vehicle_trace,'ydata',state(1:i,2));
    end
    if (trace_flag == 0)
